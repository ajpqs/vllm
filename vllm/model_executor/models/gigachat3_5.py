...
def _build_norm(
    ...
    if norm_type == "ZeroCenteredRMSNorm":
        return GigaChat35ZeroCenteredRMSNorm(hidden_size, eps=config.rms_norm_eps)
...
class GigaChat35GatedDeltaNetAttention(QwenGatedDeltaNetAttention):
    def __init__(
        ...
        self.linear_gating_type = getattr(
            config, "linear_gating_type", "gated_rmsnorm"
        )
...
class GigaChat35Attention(nn.Module):
    ...
    def forward(
        ...
        v = F.pad(v, [0, self.qk_head_dim - self.v_head_dim], value=0)
        v = v.view(-1, self.num_local_heads * self.qk_head_dim)
...
class GigaChat35DecoderLayer(nn.Module):
    def __init__(self, vllm_config: VllmConfig, prefix: str = "") -> None:
        ...
        if self.use_pre_layernorm:
            self.input_layernorm = _build_norm(
                config,
                config.hidden_size,
                quant_config=quant_config,
                prefix=f"{prefix}.input_layernorm",
            )
            self.post_attention_layernorm = _build_norm(
                config,
                config.hidden_size,
                quant_config=quant_config,
        ...
        if self.use_post_layernorm:
            self.post_self_attn_layernorm = _build_norm(
                config,
                config.hidden_size,
                quant_config=quant_config,
                prefix=f"{prefix}.post_self_attn_layernorm",
            )
            self.post_feedforward_layernorm = _build_norm(
                config,
                config.hidden_size,
                quant_config=quant_config,
...
class GigaChat35Model(nn.Module):
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        for name, loaded_weight in weights:
            if _is_extra_decoder_layer(name, self.config.num_hidden_layers):
                continue
